...
Seat: WAVE PLAN, **revision 6** - four bounded edits after the narrow
verification of section 9.3, the gate table an implementer actually binds.
**No new mechanism, and no ruling implicated**; rulings 19-24 are sound and 9.3
implements them. All rounds of
...
### 0.0000 Round 7 - four bounded edits after the narrow verification of 9.3

Three of revision 5's four failures verified closed. What remained:

| Finding | Disposition | Where |
|---|---|---|
| **B-A** - nothing required the disclosure line TO EXIST, or `cohortLabelSpread` to be called from the panel; deleting the line left the entire table green. Failure (b) in its SECOND form | **ROW ADDED** over the comment-stripped render body, plus sabotage **P20**, plus a fix to the inherited caller row's "at least one of its exports" quantifier | 9.3 |
| **M-A** - 5.5.1 still stated BOTH superseded pin forms, each of which 9.3 itself identifies as red | **BOTH MARKED SUPERSEDED IN PLACE**, with the live form named | 5.5.1 |
| **M-B** - THE PROJECTION's negative half was not computable over the region and was jointly unsatisfiable with the positive pin three rows down | **RESTATED as a pin on the PROPERTY INITIALISER**, plus the same hoist tolerance the meta argument already had | 9.3 |
| **M-C** - `snapshot-autofire.structure.test.ts` is in wave 2's set with no row, while section 7 diagnoses exactly why it needs one | **ROW ADDED** | 9.3 |
| **minors** - `LEAF IS CALLED` conflated two regions; the gated detector is parens-satisfiable by the mandated comment and is module-local; the clear row was count-satisfiable; `classified.error` is the stripped form; `:541-546` cited by raw line three times with the `if` at `:540`; literals read as free identifiers; the join key was unspecified; the canary describe is `:283-299` | **ALL EIGHT APPLIED** | 9.3, 5.5.1 |
| **wave 1 landed its scope** (`f4c792a`), and one of its residuals BINDS this plan: the region becomes 6 lines / 12 occurrences | **CARRIED**, with the reason no assertion here depends on it | 5.5.1 |
...
**AND THIS FIGURE MOVES UNDER WAVE 1 - any wave-2 assertion written against
seven lines is WRONG ON ARRIVAL.** Wave 1 is scoped and its scope doc
(`docs/a16-wave1-scope.md`, landed `f4c792a`) moves the Capture fieldset into a
new `GradingCaptureSettings.tsx`, taking the panel from 966 to 917 by both
counters with the comment count RISING 298 to 304. It reports the region
becoming **6 lines / 12 occurrences** - fewer lines, MORE occurrences, because
three of the four survivors are prop bindings that carry the identifier twice.
Measured at HEAD today the panel is still 966 with 298 comment lines
(`@(Get-Content).Count` and a `Select-String` count), so wave 1 is scoped and
not yet built, and both figures are live until it lands.

**This is exactly why no assertion in 9.3 counts occurrences.** The whitelist,
the marker-located region and the property-initialiser pins are all immune: none
of them is a function of how many times an identifier appears. The count exists
here only to show why a BAN over the region would be red - and that argument
holds at 7/10 and at 6/12 alike. Wave 2 re-measures at dispatch and does not
carry either number into a test.
...
- **NEGATIVE pin, restated as a whitelist on THE CONST'S INITIALISER** (ruling
  ...
  `runCohortMeta`), **ignoring literals** - `null`, `""` and numbers are not
  free identifiers, so the natural `lastRunCohort ? toRunCohortEntry(...) : null`
  ternary is not a violation. The mount's `entry={...}` is that `<name>`; the
  guard is `hasTrendableResults(<name>) &&`. **SUPERSEDED: "the disclosure line's
  value expression is rooted only at `<name>` or `lastRunCohort`"** - that
  excludes `cohortLabelSpread`, the helper the line must call, and is the same
  red-on-a-correct-implementation shape one prop over. The live form is 9.3's
  disclosure row, which admits the helper.
...
- **Reuse the shipped detector shapes rather than inventing them.**
  ...
  and `classTrendsMountIsGated` (`:277-281`, canary describe `:283-299`) - and
  ...
  `:198`, `:248`, `:283`, `:321`).
- **POSITIVE pin, restated - and this is the ONLY form. SUPERSEDED: "the
  `setLastRunCohort(` argument mentions `assessmentId` and `selectedCourse`"**,
  which 9.3 itself identifies as red on the natural post-leaf shape
  `setLastRunCohort(buildRunCohort(result.results, identity, meta))`. The live
  form: in the comment-stripped `handleGradeAll` body, **`buildRunCohort(`'s META
  argument** mentions `assessmentId` (or `assessmentLabel`) and `selectedCourse`,
  following a hoisted `const meta = {...}` binding. Stripping is what makes P8
  kill.
...
| **same file - THE MERGE ITSELF (ruling 19)** | `buildRunCohort(results, identity, meta)` is exercised **directly**, with `results` produced by running the real `classifyGradingResult` over a real success result and `identity` carrying real `id`/`studentName`/`assessment`. The returned cohort carries **at least one row with non-empty `rubricAreas`**, and `hasTrendableResults(toRunCohortEntry(cohort))` is **true**. Because the merge is a leaf, this test sees the real merge rather than a fixture of it |
| **same file - THE JOIN KEY IS BY `id`, NOT POSITIONAL** | `buildRunCohort` matches each result to its identity element **by `id`**, asserted on a fixture whose two arrays are in DIFFERENT orders - a positional join passes every other row on same-ordered fixtures and silently attributes one student's areas to another. Plus a **length-mismatch cell**: an identity array missing one result's `id` yields a cohort with that row **omitted**, never a row with an undefined student. Revision 5 left the join unspecified |
...
| **same file - THE ROW-STATE MAPPING, RE-DERIVED** | **TWO cases, not four**, enumerated over `classifyGradingResult`'s actual return x {areas, no areas}: `"ready"` -> `GradedResult`; `"failed"` -> `UngradedResult` with `ungraded: { kind: "grading-failed", sourceIndex, student, message: classified.error }`. **`message` comes from the CLASSIFIER, not from `row.error`** - `row.error` is outside the permitted identity projection, and `classifyGradingResult`'s failed branch already returns `error`, so a safe implementation exists and this names it. **Note deliberately: `classifyGradingResult` STRIPS `GRADING_FAILURE_PREFIX` before returning `error`, while `GradingFailedOutcome.message`'s own doc comment (`types.ts`) describes a prefixed string. The stripped form is what this plan specifies** - inert, because that comment also says "nothing in this repo branches on its text", but stated so a later reader does not read the mismatch as a defect and re-prefix it | **No `not-attempted` row is ever emitted**, and the row asserts that: `classifyGradingResult` returns only `state: "failed" \| "ready"`, so `"pending"` and `"grading"` are unreachable from this row source, and `NotAttemptedOutcome.stoppedBy` (`types.ts:142`) is `"submission-count-bound" \| "run-deadline"` - **both false of a pending recording row**, so there is no truthful value to emit. Revision 4's four-member enumeration forced impossible fixtures or a widened cohort |
...
| **same file - THE ERROR PATH CLEARS THE COHORT (ruling 23)** | after ANY non-success exit, `lastRunCohort` is **null**. **Three branches, not two**, named by symbol rather than by line: the `if (!readiness.ok) {` refusal (revision 4 missed this one - it returns before the run starts and leaves the previous cohort standing), the `"error" in result` early return, and the `catch`. Sabotage P16 |
...
| `npx vitest run .../GradingRecordingPanel.wiring.test.ts` - **THE LEAF IS CALLED (B1), TWO REGIONS** | **split, because an import statement can never appear inside a handler body**: (a) over the **WHOLE comment-stripped file**, an `import` naming `buildRunCohort` from the cohort leaf; (b) over the **comment-stripped `handleGradeAll` body**, a `buildRunCohort(` call. Modelled on the shipped `importsAndRendersClassTrendsPanel` shape, with its three canaries (true on import+call, false on a dead import, false on a local reimplementation). **Nothing in revision 4 required the panel to call the leaf at all** - it could have shipped correct, tested and unreferenced. Sabotage P17 |
| **same file - THE DISCLOSURE LINE EXISTS (B-A)** | over the **comment-stripped render body**, the panel **references `cohortLabelSpread`**. **This is the same failure as the row above, one prop over, and revision 5 left it open**: every other appearance of the disclosure line in this table constrains the LEAF's predicate or the SHAPE of a line whose existence is presupposed, so **deleting the line entirely left the whole table green** - the four-cell predicate enumeration, the whitelist, the mount row and the disclosure pin all pass on a panel that never renders it. That is failure (b) surviving in its SECOND form: ruling 20 closed "every row carries the same label", and "the line was never written" stayed open. Sabotage P20 |
| **same file - THE INHERITED CALLER ROW IS NOT ENOUGH** | wave 1's row reads "at least one of its exports", which `toRunCohortEntry` alone satisfies. Wave 2's version names **both** required references by symbol: `toRunCohortEntry` (the whitelist's initialiser) and `cohortLabelSpread` (the row above). A weaker quantifier inherited from another wave is how this class recurred |
...
| **same file - THE PROJECTION NAMES `assessment` (B1, closes P15)** | in the same region, `buildRunCohort(`'s IDENTITY argument - **following a hoisted `const identity = ...` binding, exactly as the meta argument is followed; identity is the argument MOST likely to be hoisted, since inlining a filter-map as a call argument is the unnatural shape** - contains a property initialiser `assessment:` whose value expression **names `r.assessment`** (a per-row read) and names neither `assessmentId` nor `assessmentLabel`. **The pin is on the PROPERTY INITIALISER, not on region membership (M-B).** Revision 5 forbade a bare `assessmentId` anywhere in the comment-stripped handler body - which is not computable over that region and is **jointly unsatisfiable with the positive pin three rows down, which REQUIRES `assessmentId` in the same region**. Same reason as the row above: the projection is handler code, and the leaf-side row passes its own identity array, so the leaf would copy P15's mutation through invisibly |
| **same file - THE CLEAR IS IN EVERY NON-SUCCESS BRANCH (B1, closes P16)** | in the same region, `setLastRunCohort(null)` appears in **all three** non-success branches, **each ATTRIBUTED TO ITS OWN BRANCH** rather than counted: one inside the `if (!readiness.ok) {` block, one inside the `if ("error" in result) {` block, one inside the `catch`. Cited **by symbol, never by line** - the readiness `if` is at `:540` today with its body `:541-546`, revision 5 cited `:541-546` three times, and both wave 1 and wave 2 move it. A bare count is satisfiable by three calls piled in one place. A pure leaf's unit test cannot assert anything about panel state after a failed run, which is where revision 4 filed this |
| **same file - `snapshot-autofire.structure.test.ts`'s CITATION IS RE-PINNED (M-C)** | that file is **in wave 2's published set** and revision 5 gave it no row, while section 7 diagnoses exactly why it needs one: wave 2 inserts three imports and a `useState`, shifting the auto-drain construct below them by **+4**, and section 7's own words are that it is a comment, so **it never goes red and will be missed unless named**. The row: the citations at `:28` and `:246` name the construct's post-wave line range, re-measured in-wave by `sed -n '<range>p' <panel> \| cat -n` rather than arithmetic. Diagnosing a miss and omitting its instrument is the deletion class this plan's own register forbids |
...
| **same file - THE MOUNT IS GATED (M3)** | `hasTrendableResults(<name>) &&` precedes the `<ClassTrendsPanel` tag within the same gated expression, reusing the shipped `classTrendsMountIsGated` shape (`gradingResultsExtraction.wiring.test.ts:277-281`, canary describe **`:283-299`**). **It is a module-local `function` at `:277`, not exported** - `grep -n "^export function classTrendsMountIsGated"` returns nothing while `grep -n "^function classTrendsMountIsGated"` returns `277` - so like `stripComments` it must be **DUPLICATED**, never imported across test files. **Run it over the COMMENT-STRIPPED source**: its regex `/hasTrendableResults\([^)]*\)\s*&&([\s\S]{0,400})/` would be satisfied by a hinge comment that writes the guard WITH parentheses. Latent rather than live - the shipped precedent's comment writes it without - but this plan mandates a hinge comment at the mount, so the hazard is created here. **Revision 4 asserted import-and-render and never that the mount was gated**, and `DraftedGradesTab.tsx:627` is a LIVE UNGATED PRECEDENT to copy: `grep -rn "<ClassTrendsPanel" src --include=*.tsx` returns two sites and `grep -c "hasTrendableResults" src/app/components/DraftedGradesTab.tsx` returns **0** |
...
| **P16** | **Delete `setLastRunCohort(null)` from any one of the three non-success branches**, including the one inside `if (!readiness.ok) {` | the error path | **THE CLEAR IS IN EVERY NON-SUCCESS BRANCH row**, same region, and the branch attribution is what makes deleting exactly one detectable. A pure leaf's unit test cannot assert anything about panel state after a failed run |
...
| **P19** | **Emit a `not-attempted` row for any input** | the mapping | **THE ROW-STATE MAPPING row.** `totalResults` counts graded results only (`class-trends.ts:270-273`) and nothing reads `ungradedCounts`, so a widened cohort changes no counted output - this is the mutation invisible to every other gate |
| **P20** | **Delete the disclosure line from the panel entirely** | B-A | **THE DISCLOSURE LINE EXISTS row.** Under revision 5 this mutation passed the ENTIRE table: the predicate's four-cell enumeration stayed green, the whitelist stayed green, the mount row stayed green, and the disclosure pin went vacuous. It is the same shape P17 exists for, one prop over |
| **P21** | **Join `results` to `identity` positionally**, on same-ordered fixtures | the join key | **THE JOIN KEY row**, whose differently-ordered fixture is the only thing that sees it. A positional join attributes one student's rubric areas to another and passes every other row |
