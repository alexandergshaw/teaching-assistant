...
| # | Work | State |
|---|---|---|
| 3.1 | **G3: announcement shape floor, emoji control, resource research.** Criteria complete; architect round 2 authored and under adversarial check; UX, security and reliability passes all authored and all ruled DEFECTIVE in round 1. Thirteen orchestrator rulings bind the chunk (scratchpad `g3-rulings.md`; rulings 1-2 are earlier rounds restated after I collided their numbers, 3-13 are this round's). **The reframe that matters: ask 3 is a PORT of a shipped spine, not a new build** - `gatherReplyResourcesAction` (`discussion-replies.ts:683-853`), the `vi.mock` pattern (`discussion-replies-resources.test.ts:20-28`), `useReplyResources.ts`'s try/catch plus `"error" in result` shape, `resource-search-outcome.ts`, and `deriveResourceConcepts` (`learning-resources-generator.ts:166`, materials-sized) all already exist. The floor's precedence requires amending four prompt sentences (`walkthrough-announcement-prompt.ts:156,228,234,256`) in the same change. The enforcer runs inside `draftWalkthroughAnnouncementAction`. Both controls are panel-wide with one `ta-` key each (`ta-rec-wta-emoji`, `ta-rec-wta-resources`), taking that directory's canary from 3 to 5 - `walkthrough-announcement.structure.test.ts:107` asserts exactly 3 today (`:106` is the title). Research must NOT live in `buildRequest`: it is synchronous and `regenerate` calls it too. | Criteria done, design in check, unbuilt |
| 3.3 | **H1: make screencap-to-grade as cheap as the ChatGPT equivalent.** Owner's ask, 2026-09-13: the recording-to-grade-discussion-posts workflow "should follow the same steps as if i was screencapping rubric and posts into chat gpt and providing it with instructions... allows users to effortlessly screencap rubric/posts and instantly grade". This is a WORKFLOW criticism of a shipped feature, not a new build. Measured so far: `SnapshotCaptureBar.tsx` (106 lines) is screen-share based - a "Role to arm" select, a Snap button, Start/Stop share - so capture costs a share picker plus a role arming per shot; `SnapshotGradingPanel.tsx:377` ALREADY handles `event.clipboardData`, so paste intake may exist but be undiscoverable. Six roles at `snapshot-shot.ts:29`. The open question the criteria must answer is whether this is MISSING capability or HIDDEN capability - they are different chunks. Criteria in progress. | Criteria being authored |
