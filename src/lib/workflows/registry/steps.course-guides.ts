...
// resolveContinuousWeeks/buildCourseScheduleDocx/ScheduleRow used to be
// defined in THIS file; they moved to course-schedule-docx.ts (a module with
// no `@/app/actions` import at all) so save-zip-to-course
// (steps.course-setup.storage.ts) can use the exact same schedule-docx
// rendering without pulling this file's own @/app/actions surface
// (listCourseHubAction, createPageAction, generateCourseFaqAction, etc. -
// and their transitive chain into src/lib/supabase/server.ts's
// `next/headers` import) into its bundle. Re-exported under their original
// names below so every existing caller/import of THIS file (including
// steps.course-guides.test.ts) needs no change.
import {
  type ScheduleRow,
  resolveContinuousWeeks,
  buildCourseScheduleDocx,
} from "@/lib/workflows/course-schedule-docx";

export type { ScheduleRow };
export { resolveContinuousWeeks, buildCourseScheduleDocx };
