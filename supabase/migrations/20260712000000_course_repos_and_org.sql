...
-- and self-guarding: a safe no-op on a courses table that already has these
-- columns (from the create migration), and a correct upgrade for an older table
-- that only had the single github_repo/github_branch columns.
--
-- The legacy github_repo/github_branch columns are intentionally NOT dropped, so
-- older app builds that still write them keep working during a rolling deploy.
...
-- github_repo column is present (guarded so the reference cannot error).
do $$
begin
  if exists (
