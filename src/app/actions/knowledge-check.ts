...
// The question/choice types, the 5-8 bounds, and isUsableKnowledgeCheckQuestion
// all live in @/lib/knowledge-check-shape: this is a "use server" module, and
// such a module may export nothing but async functions, so a `const` or a
// synchronous function exported from here is a BUILD error (invisible to both
// tsc and vitest). Types are erased, so re-exporting those is legal and keeps
// `import type { KnowledgeCheckQuestion } from "@/app/actions"` working for
// existing callers; the two constants and the predicate must be imported from
// the shape module directly.
export type { KnowledgeCheckChoice, KnowledgeCheckQuestion } from "@/lib/knowledge-check-shape";
