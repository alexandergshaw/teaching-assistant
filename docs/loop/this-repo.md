...
| Gate | Enforces |
|---|---|
| `src/file-size-ceiling.structure.test.ts` | **1000 lines, repo-wide over all of `src/`.** `LIMIT = 1000` at `:41` (was cited as `:30` until 2026-09-23 - re-measured with `grep -n "LIMIT = 1000" src/file-size-ceiling.structure.test.ts`). Has an `ALLOWED_OVERAGE` ratchet: each entry is pinned to the file's count when the list was written, so a listed file may shrink but fails the moment it grows. Never raise a `maxLines` to fit a file that grew. |
