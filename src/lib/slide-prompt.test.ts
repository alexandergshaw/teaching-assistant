...
describe("slide-prompt shared pedagogical contract", () => {
  ...
  //
  // AMENDED (Group A): the pin was DELIBERATELY updated again, in the same
  // commit that removed Bridge slides from both contracts. The shape lost
  // its `{ "title": "Bridge: ... to ..." }` line and nothing else, which is
  // why the length fell by 82 characters. Recorded here rather than silently
  // re-pinned: this pin catches ACCIDENTAL drift, so every intentional move
  // has to say what moved it, or the next reader cannot tell the two apart.
  // The new length/hash were read from the live constant programmatically,
  // never typed by hand.
  describe("the coding JSON shape carries the ported lecture-flow slides (Z4-AC0/AC1)", () => {
    it("SLIDE_DECK_JSON_SHAPE is byte-identical to its post-Z4-parity value", () => {
      expect(SLIDE_DECK_JSON_SHAPE.length).toBe(1789);
      expect(createHash("sha256").update(SLIDE_DECK_JSON_SHAPE).digest("hex")).toBe(
        "adabcb7735e3f9f77506ba1b08cd90ca3319103b81c5f84ba7f8021f6820ac95"
      );
    });
  ...
  // by hand.
  //
  // AMENDED (Group A): updated AGAIN, in the same commit as the deck-contract
  // audit fixes. Four deliberate edits moved it, all in this one constant:
  // the BRIDGES rule was replaced by NO TRANSITION SLIDES; a TITLE LENGTH
  // rule was added and ASSERTION TITLES rewritten to obey it (its old worked
  // example was itself 88 characters - the exact length of the worst title in
  // the audited deck); the SLIDE BUDGET formula went from "9 + concepts * 7"
  // to "10 + concepts * 6" now that the per-concept bridge is gone; and the
  // RECAP rule stopped mandating a counterfactual, which is what let a deck
  // claim Sun Microsystems would have avoided the fragmentation that led to
  // Java - inverting its own Case Study, since Sun created Java. Net +2211
  // characters. Read from the live constant programmatically, not typed.
  describe("the shared voice and flow contract", () => {
    it("SLIDE_STRUCTURE_REQUIREMENTS is byte-identical to its post-graphics-requirement value", () => {
      expect(SLIDE_STRUCTURE_REQUIREMENTS.length).toBe(22626);
  ...
  // Group A dropped "Bridge:" from this parity list - not from the coding
  // contract alone, but from BOTH, so the two kinds stay at parity by no
  // longer having the slide at all. See "NO TRANSITION SLIDES" below.
  it("the coding contract now carries the SAME lecture-flow markers the applied contract does (Z4 parity)", () => {
    for (const marker of ["Agenda:", "Section <n>:", "Recap: Where We Landed", "Appendix: Post-Lecture Practice", "SLIDE BUDGET"]) {
      expect(SLIDE_STRUCTURE_REQUIREMENTS).toContain(marker);
    }
    for (const marker of ["Agenda:", "Section 1:", "Recap: Where We Landed", "Appendix: Post-Lecture Practice"]) {
  ...
  describe("Group Z: coding-contract parity with the applied lecture-flow rules", () => {
    ...
    // Group A REVERSED the Z4-AC1 bridge port. The applied contract's V5
    // "anchor the bridge positionally" fix was ported to coding for parity;
    // both are now deleted instead, because a deck audit found every Bridge
    // slide to be filler AND their titles - two concatenated concept names -
    // could not fit the TITLE LENGTH cap the same audit produced (59-74
    // characters measured across a real deck).
    it("Group A: the coding contract has no BRIDGES rule and forbids transition slides outright", () => {
      expect(SLIDE_STRUCTURE_REQUIREMENTS.split("\n").find((line) => line.startsWith("- BRIDGES:"))).toBeUndefined();
      expect(SLIDE_STRUCTURE_REQUIREMENTS).not.toContain('"Bridge: <this concept> to <next concept>"');
      expect(SLIDE_DECK_JSON_SHAPE).not.toContain('"title": "Bridge: ... to ..."');

      const rule = SLIDE_STRUCTURE_REQUIREMENTS.split("\n").find((line) =>
        line.startsWith("- NO TRANSITION SLIDES:")
      );
      expect(rule).toBeDefined();
      // The model reaches for these unprompted once "Bridge:" is gone.
      for (const banned of ['"Bridge:"', '"Transition:"', '"Up Next:"', '"Moving On:"']) {
        expect(rule).toContain(banned);
      }
      // The hand-off the bridge carried has to survive somewhere, or this is
      // a removal that costs the deck its flow.
    ...
    it("Z4-AC2: budgets by lecture duration and slide-cost (TIME), not a slide-count formula alone, and states the figure is IN-LECTURE only", () => {
      ...
      // Was "9 + concepts * 7". Group A removed the per-concept Bridge
      // slide, so the cycle lost one slide per concept and the fixed term
      // absorbed the bridge the last concept never had.
      expect(SLIDE_STRUCTURE_REQUIREMENTS).toContain('"10 + concepts * 6" IN-LECTURE slides');
      expect(SLIDE_STRUCTURE_REQUIREMENTS).not.toContain("9 + concepts * 7");
    ...
    it("Z4-AC1 (lesson learned from RCA13/RCA20 in the applied path): the SLIDE BUDGET formula is satisfiable at every conceptCountForMinutes value (2-7) without contradiction", () => {
      ...
      // does not guarantee - "10 + concepts * 6" has no such floor (unlike
      // the applied path's old, since-fixed "process graphic needs 3 steps"
      // mistake at a 2-concept agenda).
      for (const count of [2, 3, 4, 5, 6, 7]) {
        const inLecture = 10 + count * 6;
    ...
    it("Z4-AC1 (lesson learned from RCA11 in the applied path): ASSERTION TITLES never conflicts with the Section/Agenda/Recap/Next-Week label formats", () => {
      ...
      // BRIDGES dropped out of this exclusion list with the slide itself.
      expect(SLIDE_STRUCTURE_REQUIREMENTS).toContain(
        "nor to the SECTION DIVIDERS, AGENDA SLIDE, RECAP, or NEXT WEEK title formats"
      );
    });
