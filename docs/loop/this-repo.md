...
| Gate | Enforces |
|---|---|
| `src/file-size-ceiling.structure.test.ts` | **1000 lines, repo-wide over all of `src/`.** `LIMIT = 1000` at `:30`. Has an `ALLOWED_OVERAGE` ratchet: each entry is pinned to the file's count when the list was written, so a listed file may shrink but fails the moment it grows. Never raise a `maxLines` to fit a file that grew. |
