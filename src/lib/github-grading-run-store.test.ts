...
describe("serializeGithubGradingRun / parseStoredGithubGradingRun - round trip", () => {
  ...
  it("round-trips a result with no gradedRepo/gradedRef/userId (not a GitHub- or Canvas-sourced row)", () => {
    ...
    const json = serializeGithubGradingRun({ run, gradedAt: "2026-08-24T12:00:00.000Z", lastGradedFolder: "" });
    const restored = parseStoredGithubGradingRun(json);
    expect(restored).not.toBeNull();
...
describe("parseStoredGithubGradingRun - R2.3 never trust stored data", () => {
  ...
  it("returns null when gradedAt or lastGradedFolder is missing or the wrong type", () => {
    const run = fixtureRun();
    expect(parseStoredGithubGradingRun(JSON.stringify({ lastGradedFolder: "week-1", run }))).toBeNull();
    expect(
      parseStoredGithubGradingRun(JSON.stringify({ gradedAt: 123, lastGradedFolder: "week-1", run }))
    ).toBeNull();
    expect(
      parseStoredGithubGradingRun(JSON.stringify({ gradedAt: "2026-08-24T12:00:00.000Z", lastGradedFolder: null, run }))
    ).toBeNull();
  });

  it("returns null when run is missing or not an object", () => {
    expect(
      parseStoredGithubGradingRun(JSON.stringify({ gradedAt: "2026-08-24T12:00:00.000Z", lastGradedFolder: "" }))
    ).toBeNull();
    expect(
      parseStoredGithubGradingRun(
        JSON.stringify({ gradedAt: "2026-08-24T12:00:00.000Z", lastGradedFolder: "", run: "nope" })
      )
    ).toBeNull();
  });
...
describe("loadStoredGithubGradingRun / persistGithubGradingRun", () => {
  ...
  it("round-trips a persisted run", () => {
    const run = fixtureRun();
    persistGithubGradingRun({ run, gradedAt: "2026-08-24T12:00:00.000Z", lastGradedFolder: "week-1" });
  ...
  it("does nothing when persisting with window undefined (SSR-safe write)", () => {
    delete (globalThis as { window?: unknown }).window;
    expect(() =>
      persistGithubGradingRun({ run: fixtureRun(), gradedAt: "2026-08-24T12:00:00.000Z", lastGradedFolder: "" })
    ).not.toThrow();
  ...
  it("a quota failure loses persistence for only that run - a later successful write still works", () => {
    ...
    persistGithubGradingRun({ run: fixtureRun(), gradedAt: "2026-08-24T12:00:00.000Z", lastGradedFolder: "" });
    fakeStorage.throwOnSet = false;
    persistGithubGradingRun({ run: fixtureRun(), gradedAt: "2026-08-24T13:00:00.000Z", lastGradedFolder: "week-2" });
    const loaded = loadStoredGithubGradingRun();
    expect(loaded).not.toBeNull();
