...
| # | Gap | Note | From |
|---|---|---|---|
| 5.1 | A hung exemplar fetch disables Generate indefinitely. `WalkthroughAnnouncementPanel.tsx:793` disables it on `savedExemplarsLoading`, and the `Promise.all` at `:241-244` has no timeout - **either** call in it hanging does this, so a fix that times out only one leaves the hole open. Two loading signals exist (`AnnouncementDraftSlot.tsx:103` and `WalkthroughAnnouncementPanel.tsx:686`), neither of which is an error state. | Green suite, dead button | `b3701cd` |
...
| 5.7 | **The ceiling gate cites a rule that no longer exists where it says.** `src/file-size-ceiling.structure.test.ts:6` quotes `docs/DEV_LOOP.md` for "The 1000-line ceiling on every touched file", and `:124` repeats the attribution inside the FAILURE MESSAGE an engineer reads at the moment the gate fires. `grep -c "1000" docs/DEV_LOOP.md` returns **0** - `338962a` rewrote that card. Both are the only two citers (`grep -rn` over `src/`). The rule itself is sound and gated; only its stated home is wrong, which is why no test catches it. Fix by re-pointing both to the test's own header, or by restoring the sentence to `DEV_LOOP.md`. | Two lines, one file | `338962a` |
