...
/**
 * Research current events surrounding a deck's topics and generate a report.
 * Calls the LLM with web search enabled to find recent developments.
 * Returns the report and a count of sources cited (for display in the step summary).
 */
export async function researchCurrentEventsAction(
  deckText: string,
  recentWindow: string,
  provider: LlmProvider = "gemini"
): Promise<{ report: string; sourceCount: number } | { error: string }> {
  try {
    await requireOwner();
...
Your task: Identify the deck's major topics. For each topic, search for and report current events, news, and recent developments from ${window} that are relevant to that topic. Include concrete dates and explain why each item matters for a lecture on that topic. Ground your claims in the sources you find — do not fabricate citations or sources.

Structure your response with:
- # Major Topics (list the topics)
- ## Topic: <name> (one section per major topic with current events for that topic)

Use Markdown headings (# and ##) so the document structures correctly. Be specific about dates and concrete examples.`;
    ...
    if (!result.ok) {
      return { error: `Research failed: HTTP ${result.status} — ${result.body.slice(0, 200)}` };
    }

    const reportText = result.text.trim();
    if (!reportText) {
      return { error: "The model returned an empty report. Try again." };
    }
    ...
    if (result.sources && result.sources.length > 0) {
      ...
      for (const source of uniqueSources) {
        report += `- ${source.title}: ${source.uri}\n`;
      }
      sourceCount = uniqueSources.length;
    } else {
      report += "\n\n## Sources\n";
      report += "No web sources were returned for this report.\n";
    }

    return { report, sourceCount };
  } catch (err) {
