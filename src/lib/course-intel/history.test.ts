...
type Row = Database["public"]["Tables"]["course_intel_answers"]["Row"];
...
// most functions here issue exactly one, and clearCourseIntelAnswers (a
// count read, then a delete) issues two.
// ============================================================================
...
function fakeSupabase(resolveValues: unknown[]) {
  ...
  function from(table: string) {
    ...
    const chainable = ["insert", "select", "delete", "eq", "order"] as const;
...
describe("listCourseIntelAnswers - tenant boundary", () => {
  it("filters on both user_id and course_id", async () => {
...
describe("exportCourseIntelAnswers - tenant boundary", () => {
  it("filters on both user_id and course_id", async () => {
    const { client, calls } = fakeSupabase([{ data: [SAVED_ROW], error: null }]);
  ...
  it("orders by created_at ascending (chronological, for a full export)", async () => {
    const { client, calls } = fakeSupabase([{ data: [], error: null }]);
