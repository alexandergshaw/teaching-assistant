...
// 1000-line cap after the CODING deck contract was brought to parity with
...
// checks that verify parity between the two course kinds. See
// slide-prompt.test.ts for the CODING contract's own tests (including its
// hash/length pins), the shape helpers, and enforceNoCodeForApplied; see
// slide-prompt.structural-guard.test.ts for the structural consistency
// guard. No test was weakened, merged, or deleted by this split - every
// assertion that lived in slide-prompt.test.ts before this split still
// runs, here or there.
...
describe("slide-prompt shared pedagogical contract", () => {
  ...
  // Applied-side counterpart of "the coding variant composes
  // PLAIN_LANGUAGE_CONTRACT verbatim" in slide-prompt.test.ts's "the shared
  // voice and flow contract" describe block (the coding-side hash pin lives
  // there, alongside the coding variant's own version of this check).
  ...
  // bullets. The coding contract is untouched by this - it is already
  // pinned byte-for-byte in slide-prompt.test.ts (length + sha256), which
  // is the strongest guarantee that adding graphics to the applied side
  // left it alone (see that file's "the coding contract allows an optional
  // graphic..." test); the checks below add a second, human-readable guard
  ...
  // tests) - this file only pins the PROMPT side of P3. (P3-AC5, the
  // coding-side half, lives in slide-prompt.test.ts.)
  ...
  // already sets ("Absent a concept plan, ...", see slide-prompt.test.ts) -
  // never leaving the model to fabricate a concept plan, a duration, or a
  // next week's topic just to satisfy the letter of the rule.
