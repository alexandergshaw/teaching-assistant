...
| Cost control | 74 of 135 LLM-calling files set `maxOutputTokens`; the rest inherit `DEFAULT_MAX_OUTPUT_TOKENS = 700`; `src/lib/llm.ts:503` normalises the cap centrally rather than at each of roughly 78 call sites | A shared default, not a feature-specific guarantee. Also a bad exemplar historically: `DEFAULT_MAX_SUBMISSIONS` is recorded elsewhere in this repo's own notes as a silent-truncation collision, not an advantage to claim. Its value is 40, not the 5 this card asserted until 2026-09-23 - the stale figure had propagated into a seats.md argument that rested on the two numbers coinciding. |
