...
## 206. A cartridge this app builds can no longer be re-uploaded as source material

Backlog group G, closing the residual recorded in entry 202 AC5.

Entry 202 made the Files tab LABEL app-generated cartridges "Generated by Course Build", but
that label is EXTERNAL metadata: `CourseMaterialFile.generated`, a database column. Download
the file and upload it again and the column is gone, so the app treats its own output as a
fresh instructor export and builds a course from material it invented itself. The label
solved the display problem and not the round-trip one.

### The stamp

New pure module `cartridge-import-stamp.ts` holds the marker vocabulary, imported by BOTH the
writer and the reader so neither duplicates it. The stamp is a dedicated file at the archive
root, `ta-cartridge-generated.json`, carrying `app`, `generatedBy`, `kind`, `stampVersion`,
`generatedAt` and the course/week title. Nothing user-identifying.

Two decisions worth recording:

It is NOT placed in `course_settings/`, which is a Canvas-owned namespace with unpredictable
real content. It is NOT referenced by any `<resource>` in the manifest either, which is what
makes it inert: manifest-driven LMS import never looks at it, so a real Canvas or Blackboard
import carries it through as an ordinary extra file. Verified by grep - the stamp constant
appears in the two `zip.file(...)` writer calls and nowhere near the manifest builder.

`readCartridgeStamp` never throws and only a POSITIVE match changes behaviour. An unstamped
cartridge is treated exactly as it was before. This is the safe direction: failing to
recognise our own output costs one bad course build, while misclassifying a real instructor
export costs the instructor their actual material.

### The false-positive proof is the part that mattered

Checked against the real 61MB Canvas export at
`26ss-info-1020-2a-computer-science-principles-export (1).imscc`: 969 entries, ZERO matching
`/ta-cartridge|teaching-assistant|generated/i`. Verified independently of the agent's report,
with a canary asserting the search could find `imsmanifest.xml` - otherwise a clean result
would only prove the scan was blind.

The suite runs `detectAppGeneratedCartridge` and `parseCartridgeBlob` against that real file
directly, guarded with `it.skipIf(fs.existsSync)` so the suite stays portable. CONFIRMED
those two tests actually RAN here rather than skipping (167ms and 229ms), because a skipped
test would have made the whole proof vacuous on the one machine that can perform it.
`parseCartridgeBlob` still reads all 12 modules correctly, so the stamp read did not disturb
the existing parse.

A portable synthetic fixture mirrors that export's `course_settings/` file list and manifest
namespace, so the no-false-positive guarantee still has coverage on a machine without the
real file.

### Verification

23 tests. SABOTAGE-CHECKED three ways, each reverted after confirming red: writer stops
stamping (5 positive-detection tests red), detector short-circuits to true after any
successful zip load (exactly the 3 false-positive tests red, including the real-export one),
and `JSON.parse` without try/catch (both malformed-input tests red).
