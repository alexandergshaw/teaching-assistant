...
// scheduledReleaseGroup (and the RELEASE_DATE_UNPERSISTED constant it alone
// used) moved to ./bulkBarGroupCatalog.release.ts, for the same 1000-line-
// ceiling reason the "Generate" group moved to ./bulkBarGroupCatalog.generate.ts -
// see that file's own header comment for the full split rationale and why its
// one new constant (MODAL_OPENER_UNPERSISTED) is a local duplicate rather than
// an import back from here.
import { scheduledReleaseGroup } from "./bulkBarGroupCatalog.release";
...
// scheduledReleaseGroup lives in ./bulkBarGroupCatalog.release.ts, imported
// above and placed in BULK_BAR_GROUPS below at the exact position it has
// always occupied (last). See that file's own header for the full reasoning
// behind every declaration it carries.
