...
export function useAvatarStudio(): UseAvatarStudioReturn {
  ...
  const startCapturePreview = useCallback(async () => {
    ...
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia({
        ...
        audio: micId && micId !== "off" ? { deviceId: { exact: micId } } : true,
      });
  ...
  }, [cameraId, micId, stopRecorderAndTracks]);
  ...
  }, [reviewBlob, supabase, user, meetsMinDuration, withinSizeCap, mimeChoice, reviewDurationSec]);
