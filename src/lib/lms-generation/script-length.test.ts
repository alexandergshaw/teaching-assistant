...
describe("resolveScriptMinutes", () => {
  ...
  it("reads a localStorage-shaped string, including surrounding whitespace", () => {
    expect(resolveScriptMinutes(" 10 ")).toBe(10);
  });
  ...
  it("defaults a value that is IN RANGE but was never an offered option", () => {
    ...
    // 7 would sail through generateLectureScriptAction's own 1-30 check, so a
    // range test here would pass it through and the select would then render
