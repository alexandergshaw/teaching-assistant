...
| T3 | merge's new-entry factory | literal `{ name, text, rowId, dismissed }` | `grading-capture-sync.ts:142` |
| T4 | merge's fold branches | `{ ...s, text: joined }` / `{ ...s, text: submission.text }` | `grading-submission-merge.ts:417-420` **(NEW)** |
| T5 | the blank row builder | 12-field literal | `grading-capture-sync.ts:69-82` |
| T6 | the refresh branch | `{ ...existingRow, studentName, submissionText }` | `grading-capture-sync.ts:162` |
| T7 | the row type | `interface GradingRow` | `grading-row.ts:114-200` |
| T8 | the codec | `toWire` 16 keys / `fromWire` | `grading-row-serialization.ts:137-171` and `:180+` |
| T9 | the reload seed | 4-field literals, live and tombstone | `grading-capture-tombstones.ts:129-134`, `:137` |
| T10 | the panel's grade map | `{ id, studentName, submissionText }` | `GradingRecordingPanel.tsx:561-565` |
| T11 | the grade action's parameter type | `ReadonlyArray<{ id; studentName; submissionText }>` | `grading-submission-grade.ts:120` **(NEW)** |

Then the composer itself: `buildGradingRecordingPrompt`
(`grading-feedback-prompt.ts:101-108`), called at
`grading-submission-grade.ts:149-154` with four positional arguments.
...
`grading-row-serialization.test.ts:620-641` `EXPECTED_WIRE_KEYS`, a hand-written
16-key list compared with `expect(Object.keys(wire)).toEqual(EXPECTED_WIRE_KEYS)`
...
`grading-capture-sync.ts:162` already carries every non-read field of an
existing row forward untouched (that file's own header, `:38-41`: "Only the read
fields (studentName/submissionText) are refreshed here"). So a confirmed kind
...
| Production builders | 2 | `grading-capture-sync.ts:68` `blankGradingRow`, `src/lib/course-intel/offline-payload.ts:178` `buildGradingRow` |
| Bare literals | 4 | the frozen oracle at `grading-row-serialization.test.ts:388`, four rows (`nameMatch` at `:392`, `:411`, `:434`, `:453`) |
| **Total** | **15** | |
...
`buildGradingRecordingPrompt` (`grading-feedback-prompt.ts:101-108`) is a pure
leaf with no `callLlm`, no `requireOwner` and no I/O (that file's own header,
`:12-16`), and it returns the request body. It gains a `kind` parameter and
...
fragment of the submission. `grading-recording-log.test.ts:56-82` (the
`buildGradingRecordingLogRowEntry` describe, whose `expect(entry).toEqual({...})`
at `:72` is an exact-object assertion) pins the entry's key set and will red on
the addition; bump it in the same commit.
...
| `src/app/actions/grading-submission-grade.ts` | T11 and the `buildGradingRecordingPrompt` call at `:149-154` |
...
**Total: 45 paths** (3 new + 26 + 4 actions + 7 + 4 walkers/generated, with
`GradingTable.module.css` counted once). Above Ruling N's floor of about 35, as
...
here**, per the brief. The one coupling it names - `grading-feedback-prompt.ts:78`
calling `buildSystemPrompt("", rubricText, criteria)` with three positional
arguments, against A11's defaulted fifth parameter - is unaffected by A8-R:
A8-R's change to that file is to `buildGradingRecordingPrompt` (`:101-108`), a
different function, and A8-R does not add an argument to the `:78` call.
...
**Baseline.** `find src -name "*.test.ts" | wc -l` returns **1071** - my own
instrument, run in this checkout today, agreeing with the kept round-1 figure.
The TEST count (round 1: 21297) cannot be produced without running the suite,
...
| RES-5 | The suite test COUNT baseline (round 1: 21297). Not re-measured by this pass, because running the suite writes `docs/css-orphans.md` and this pass may write one file | the wave-0 gate | `npm test`, recording the "Tests N passed" line | before wave 1 starts |
...
| RES-7 | Whether Ruling I's A11/A8-R disjointness still holds against this revision's 45 paths rather than round 1's 18 | the orchestrator | `sort` piped to `uniq -d` over A11's path list and section 8's, with a self-intersect canary | before either chunk is dispatched |
